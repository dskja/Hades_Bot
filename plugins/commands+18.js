...
if (!global.db.data.chats[m.chat].nsfw) throw `🚫 Gruppe unterstützt kein NSFW \n\n Aktivieren Sie es mit \n*${usedPrefix}enable* nsfw`
...
conn.sendButton(m.chat, `${command}`.trim(), author, res, [['🔄 NÄCHSTES 🔄', `/${command}`]], m)}
...
conn.sendButton(m.chat, `${command}`.trim(), author, url, [['🔄 NÄCHSTES 🔄', `/${command}`]], m)}
