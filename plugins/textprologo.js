...
if (!effect) throw '*¿Korrekte Verwendung des Befehls?*\n—◉ /logo (Effekt) (text)\n*Beispiel:*\n—◉ /logo 3d-deep-sea-metal Yassin\n\n*[❗]  wenn zwei Texte erforderlich sind, verwenden Sie:*\n—◉ /logo (Effekt) (text1|text2)\n*Beispiel:*\n—◉ /logo Wolf-Logo-Galaxy HadesBot|Yassin\n\n*<Liste Effekte/>*\n\n° ඬ⃟📝 /logo ' + effects.map(v => v.title).join('\n° ඬ⃟📝 /logo *')
effect = effect.toLowerCase()
if (!effects.find(v => (new RegExp(v.title, 'gi')).test(effect))) throw `*dieser ${effect} kann dies nicht finden*`
let text = txt.replace(new RegExp(effect, 'gi'), '').trimStart()
if (text.includes(split)) text = text.split(split)
text = Array.isArray(text) ? text : [text]
let res = await textpro(effect, ...text)
if (typeof res == 'number') throw res == -1 ? `*[❗𝐈𝐍𝐅𝐎❗]  ${effect} wurde nicht gefunden*` : `*korrektes Format verwenden ${usedPrefix + command} ${effect} ${new Array(res).fill('texto').map((v, i) => v + (i ? i + 1 : '')).join('|')}*`
let result = await axios.get(res, {
responseType: 'arraybuffer'
})
await conn.sendFile(m.chat, result.data, 'Error.jpg', `*VERARBEITUNG!!*\n*EFFEKT: ${effect}*`, m)
}
handler.help = ['logos']
handler.tags = ['logo']
